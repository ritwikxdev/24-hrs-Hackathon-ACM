...
content = content.replace("content: 'ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦';", "content: '\\2713';")
content = content.replace("content: 'âœ\"';", "content: '\\2713';")
content = content.replace("AB-FDA ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â  AI-powered Ayushman Bharat Fraud Detection & Analytics Platform", "AB-FDA — AI-powered Ayushman Bharat Fraud Detection & Analytics Platform")
content = content.replace("AB-FDA â€” AI-powered Ayushman Bharat Fraud Detection & Analytics Platform", "AB-FDA — AI-powered Ayushman Bharat Fraud Detection & Analytics Platform")
...
clean_body = """<body>
  ...
  <section class="hero">
    ...
    <div class="hero-inner">
      <div class="hero-content">
        <div class="hero-tag fade-up da1"><span class="ping-dot"></span>AI Fraud Detection — Active</div>
        ...
        <p class="hero-desc fade-up da3">
          Real-time anomaly scoring powered by Isolation Forest ML — shielding government healthcare claims from billing fraud, upcoding, and systemic abuse without friction.
        </p>
  ...
  <div class="stats-row">
    <div class="stats-grid">
      <div class="stat-card total reveal d1"><span class="stat-icon">📊</span><div class="stat-value" id="kpiTotal">—</div><div class="stat-label">Total Claims</div></div>
      <div class="stat-card high reveal d2"><span class="stat-icon">🚨</span><div class="stat-value" id="kpiHigh">—</div><div class="stat-label">High Risk</div></div>
      <div class="stat-card medium reveal d3"><span class="stat-icon">⚠️</span><div class="stat-value" id="kpiMedium">—</div><div class="stat-label">Medium Risk</div></div>
      <div class="stat-card low reveal d4"><span class="stat-icon">✅</span><div class="stat-value" id="kpiLow">—</div><div class="stat-label">Low Risk</div></div>
      <div class="stat-card amount reveal d5"><span class="stat-icon">💰</span><div class="stat-value" id="kpiAmount">—</div><div class="stat-label">Total ₹ Claimed</div></div>
    </div>
  </div>
  ...
  <section id="platform" class="tabs-section">
    <div class="tabs-section-inner">
      <h2 class="tabs-section-title reveal d1">Our end-to-end platform covers the entire healthcare fraud lifecycle — from claim submission to risk assessment.</h2>
      ...
      <div id="tab-risk" class="tab-content">
        <div class="tab-text">
          ...
          <ul class="tab-feature-list">
            ...
            <li>Per-claim fraud score (0–100)</li>
    ...
    <section id="analytics">
      ...
      <div class="charts-grid">
        <div class="chart-card reveal d3">
          <h3>Claimed Amount by Hospital <span style="font-weight:500;color:var(--muted);font-size:.65rem;text-transform:none;letter-spacing:0;margin-left:4px">· color = risk level</span></h3>
  ...
  <footer class="footer">
    <div class="footer-inner">
      ...
      <div class="footer-bottom">
        <p>© 2026 AB-FDA. Built for ACM Hackathon.</p>
  ...
  <script>
    ...
    async function fetchClaims() {
      try {
        ...
        $('#lastSync').textContent = `Last loaded ${new Date().toLocaleTimeString()} · ${allClaims.length} records`;
        updateDashboard();
      } catch (err) {
        console.error(err);
        toast('Failed to load claims — is the backend running on port 8000?', 'error');
        showEmpty();
      }
    }
